...
public class MainActivity extends AppCompatActivity {
    ...
    private void onButtonClick(Button button) {
        if (button.getText().toString().equals("")) {
            ...
            if (checkForWinner()) {
                String winner = isPlayerX ? "Хрестики" : "Нулики";
                Toast.makeText(this, winner + " перемогли!", Toast.LENGTH_SHORT).show();
                resetGame();
            } else if (isBoardFull()) {
                Toast.makeText(this, "Отакої! Нічія!", Toast.LENGTH_SHORT).show();
                resetGame();
            } else {
                isPlayerX = !isPlayerX;
            }
